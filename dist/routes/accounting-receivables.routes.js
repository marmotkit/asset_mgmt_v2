...
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 20, status, customerId, startDate, endDate, search } = req.query;
        ...
        // 取得資料
        const [receivables] = await connection_1.default.query(`
            SELECT 
...
router.post('/', async (req, res) => {
    try {
        const { customer_id, customer_name, invoice_number, amount, due_date, description } = req.body;
        // 驗證必填欄位
