...
router.get('/', async (req, res) => {
    try {
        ...
        // 取得資料
        const [closings] = await connection_1.default.query(`
            SELECT 
...
router.post('/close', async (req, res) => {
    try {
        ...
        `, { bind: [closing_year, closing_month] });
        const totalDebit = parseFloat(journalStats[0].total_debit);
        const totalCredit = parseFloat(journalStats[0].total_credit);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        // 如果已結帳，只允許修改備註
        if (existing[0].status === 'closed' && status && status !== 'closed') {
...
router.get('/check/:year/:month', async (req, res) => {
    try {
        ...
        const totalDebit = parseFloat(journalStats[0].total_debit);
        const totalCredit = parseFloat(journalStats[0].total_credit);
        const entryCount = parseInt(journalStats[0].entry_count);
        const balance = totalDebit - totalCredit;
        res.json({
