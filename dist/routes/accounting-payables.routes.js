...
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 20, status, supplierId, startDate, endDate, search } = req.query;
        ...
        // 取得資料
        const [payables] = await connection_1.default.query(`
            SELECT 
...
router.post('/', async (req, res) => {
    try {
        const { supplier_id, supplier_name, invoice_number, amount, due_date, description } = req.body;
        // 驗證必填欄位
...
router.get('/overdue/list', async (req, res) => {
    try {
        const [overdue] = await connection_1.default.query(`
            SELECT 
                id, supplier_id, supplier_name, invoice_number, amount,
