...
router.post('/', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const { investmentId, year, month, amount, status, startDate, endDate, renterName, renterTaxId, payerName, paymentMethod, paymentDate, note } = req.body;
        // 處理日期欄位，空字串設為 NULL
        ...
        // 直接返回插入的結果
        const insertedRentalPayment = result[0];
        if (insertedRentalPayment && insertedRentalPayment.id) {
            const newRentalPayment = await connection_1.default.query(`
                SELECT 
                    rp.*,
                    i.name as investment_name,
                    i.type as investment_type,
...
router.put('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        // 獲取更新後的租金收款項目
        const updatedRentalPayment = await connection_1.default.query(`
            SELECT 
                rp.*,
                i.name as investment_name,
                i.type as investment_type,
...
router.post('/generate', async (req, res) => {
    try {
        ...
        `;
        const existingResult = await connection_1.default.query(existingQuery, {
        ...
        `;
        const rentalStandards = await connection_1.default.query(standardsQuery, {
...
router.delete('/clear/:year/:month?', async (req, res) => {
    try {
        ...
        let replacements = { year: parseInt(year) };
        if (month) {
