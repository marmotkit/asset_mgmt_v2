...
router.get('/', async (req, res) => {
    try {
        ...
        const profitSharingStandards = await connection_1.default.query(query, {
            replacements,
...
router.post('/', async (req, res) => {
    try {
        const { investmentId, type, value, minAmount, maxAmount, startDate, endDate, note } = req.body;
        // 處理數值和日期欄位，空值設為 NULL
        ...
        // 直接返回插入的結果
        const insertedProfitSharingStandard = result[0];
        if (insertedProfitSharingStandard && insertedProfitSharingStandard.id) {
            const newProfitSharingStandard = await connection_1.default.query(`
                SELECT 
                    pss.*,
                    i.name as investment_name,
                    i.type as investment_type
...
router.put('/:id', async (req, res) => {
    try {
        ...
        // 獲取更新後的分潤標準
        const updatedProfitSharingStandard = await connection_1.default.query(`
            SELECT 
                pss.*,
                i.name as investment_name,
                i.type as investment_type
