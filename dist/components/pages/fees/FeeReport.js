...
const xlsx_full_min_1 = __importDefault(require("xlsx/dist/xlsx.full.min"));
const FeeReport = () => {
    ...
    const handleExportExcel = () => {
        ...
        // 創建工作表
        const ws = xlsx_full_min_1.default.utils.json_to_sheet(excelData);
        const wb = xlsx_full_min_1.default.utils.book_new();
        xlsx_full_min_1.default.utils.book_append_sheet(wb, ws, '會費收款報表');
        // 下載檔案
        const fileName = selectedYear === 'all' ?
            '會費收款總表.xlsx' :
            `${selectedYear}年度會費收款總表.xlsx`;
        xlsx_full_min_1.default.writeFile(wb, fileName);
    };
