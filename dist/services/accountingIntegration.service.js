...
exports.accountingIntegrationService = {
    ...
    syncFeeReceivables: async () => {
        try {
            ...
            // 先清理舊的會費應收帳款
            await exports.accountingIntegrationService.clearFeeReceivables();
            // 取得會費管理的所有項目，然後篩選待收款
            const feeResponse = await api_service_1.apiClient.get('/fees');
            const allFees = feeResponse.data || [];
            // 篩選待收款和逾期的會費
            const pendingFees = allFees.filter(fee => fee.status === '待收款' ||
                fee.status === '逾期' ||
                fee.status === 'pending' ||
                fee.status === 'overdue');
            ...
            for (const fee of pendingFees) {
                try {
                    ...
                    const response = await api_service_1.apiClient.post('/accounting/receivables', receivableData);
    ...
    clearFeeReceivables: async () => {
        try {
            ...
            const response = await api_service_1.apiClient.get('/accounting/receivables?search=會費');
            const feeReceivables = response.data.receivables || [];
            for (const receivable of feeReceivables) {
                try {
                    await api_service_1.apiClient.delete(`/accounting/receivables/${receivable.id}`);
    ...
    syncRentalReceivables: async () => {
        ...
        try {
            ...
            // 先清理舊的租金應收帳款
            await exports.accountingIntegrationService.clearRentalReceivables();
            // 嘗試不同的租金 API 路徑
            let rentalData = [];
            try {
                // 嘗試租金收款 API
                const rentalResponse = await api_service_1.apiClient.get('/rental-payments');
                ...
                try {
                    // 嘗試投資管理 API
                    const investmentResponse = await api_service_1.apiClient.get('/investments');
                    const investments = investmentResponse.data || [];
                    // 從投資項目中提取租金資料
                    for (const investment of investments) {
                        if (investment.type === 'rental' || ((_a = investment.description) === null || _a === void 0 ? void 0 : _a.includes('租賃'))) {
                            // 為每個投資項目生成年度租金記錄
            ...
            // 篩選待收款和逾期的租金
            const pendingRentals = rentalData.filter(rental => rental.status === '待收款' ||
                rental.status === '逾期' ||
    ...
    clearRentalReceivables: async () => {
        try {
            ...
            const response = await api_service_1.apiClient.get('/accounting/receivables?search=租金');
    ...
    syncMemberProfitPayables: async () => {
        try {
            ...
            // 先清理舊的分潤應付帳款
            await exports.accountingIntegrationService.clearProfitPayables();
            // 取得會員分潤管理的待付款項目（全年度）
            const profitResponse = await api_service_1.apiClient.get('/member-profits?status=待付款');
            ...
            for (const profit of pendingProfits) {
                try {
                    ...
                    const response = await api_service_1.apiClient.post('/accounting/payables', payableData);
    ...
    clearProfitPayables: async () => {
        try {
            ...
            const response = await api_service_1.apiClient.get('/accounting/payables?search=會員分潤');
            const profitPayables = response.data.payables || [];
            for (const payable of profitPayables) {
                try {
                    await api_service_1.apiClient.delete(`/accounting/payables/${payable.id}`);
    ...
    syncAllAccountingData: async () => {
        try {
            ...
            const [feeReceivables, rentalReceivables, profitPayables] = await Promise.all([
                exports.accountingIntegrationService.syncFeeReceivables(),
                exports.accountingIntegrationService.syncRentalReceivables(),
                exports.accountingIntegrationService.syncMemberProfitPayables()
            ]);
            console.log('完整同步完成:', {
    ...
    // 5. 月結時自動同步資料
    syncBeforeMonthlyClosing: async (year, month) => {
        var _a, _b, _c, _d;
        try {
            console.log(`執行月結前資料同步 (${year}年${month}月)...`);
            // 先同步所有會計資料
            await exports.accountingIntegrationService.syncAllAccountingData();
            // 檢查該月份的資料完整性
            const journalResponse = await api_service_1.apiClient.get(`/accounting/journal?year=${year}&month=${month}`);
            const receivablesResponse = await api_service_1.apiClient.get(`/accounting/receivables?year=${year}&month=${month}`);
            const payablesResponse = await api_service_1.apiClient.get(`/accounting/payables?year=${year}&month=${month}`);
            const summary = {
                journalCount: journalResponse.data.length || 0,
                receivablesCount: ((_a = receivablesResponse.data.receivables) === null || _a === void 0 ? void 0 : _a.length) || 0,
                payablesCount: ((_b = payablesResponse.data.payables) === null || _b === void 0 ? void 0 : _b.length) || 0,
                totalReceivables: ((_c = receivablesResponse.data.receivables) === null || _c === void 0 ? void 0 : _c.reduce((sum, item) => sum + parseFloat(item.amount), 0)) || 0,
                totalPayables: ((_d = payablesResponse.data.payables) === null || _d === void 0 ? void 0 : _d.reduce((sum, item) => sum + parseFloat(item.amount), 0)) || 0
            };
