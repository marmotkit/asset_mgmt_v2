...
class ApiService {
    ...
    static saveInvestmentsToStorage() {
        localStorage.setItem('investments', JSON.stringify(ApiService.investments));
    }
    ...
    static async createInvestment(investment) {
        ...
        if (investment.type === 'movable') {
            newInvestment = await ApiService.createMovableInvestment(investment);
        }
        else if (investment.type === 'immovable') {
            newInvestment = await ApiService.createImmovableInvestment(investment);
        }
        else {
            // 默認為動產投資
