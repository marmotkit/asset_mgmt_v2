...
router.get('/income-statement', async (req, res) => {
    try {
        ...
        // 計算總收入
        const totalRevenue = revenue.reduce((sum: number, item: any) => sum + parseFloat(item.balance), 0);

        // 計算總費用
        const totalExpenses = expenses.reduce((sum: number, item: any) => sum + parseFloat(item.balance), 0);
...
router.get('/balance-sheet', async (req, res) => {
    try {
        ...
        // 計算總資產
        const totalAssets = assets.reduce((sum: number, item: any) => sum + parseFloat(item.balance), 0);

        // 計算總負債
        const totalLiabilities = liabilities.reduce((sum: number, item: any) => sum + parseFloat(item.balance), 0);

        // 計算總權益
        const totalEquity = equity.reduce((sum: number, item: any) => sum + parseFloat(item.balance), 0);
...
router.get('/cash-flow', async (req, res) => {
    try {
        ...
        // 計算各類現金流量總額
        const operatingCashFlow = operatingActivities.reduce((sum: number, item: any) => sum + parseFloat(item.net_cash_flow), 0);
        const investingCashFlow = investingActivities.reduce((sum: number, item: any) => sum + parseFloat(item.net_cash_flow), 0);
        const financingCashFlow = financingActivities.reduce((sum: number, item: any) => sum + parseFloat(item.net_cash_flow), 0);
...
router.get('/trial-balance', async (req, res) => {
    try {
        ...
        // 計算總借貸
        const totalDebit = trialBalance.reduce((sum: number, item: any) => sum + parseFloat(item.total_debit), 0);
        const totalCredit = trialBalance.reduce((sum: number, item: any) => sum + parseFloat(item.total_credit), 0);
