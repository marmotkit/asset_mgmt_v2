...
## v6.1 版本更新（2025/06/XX）

### 本次主要修正與新功能
- **會費歷史記錄全面雲端串接**
  - 歷史記錄頁面資料來源改為雲端 API，完全同步資料庫
  - 匯出報表功能改為直接抓取雲端資料，確保匯出內容與畫面一致
- **表格欄位排序功能**
  - 會費歷史記錄表格支援點擊表頭排序（會員編號、姓名、金額、到期日等）
  - 排序箭頭提示目前排序狀態，支援升降序切換
- **資料渲染防呆與效能優化**
  - 排序時自動處理 undefined/null，避免資料被過濾掉
  - 修正欄位名稱一致性，確保所有資料正確顯示
- **其他細節優化**
  - 修正匯出報表欄位與畫面一致
  - 增加除錯 log，方便開發與維護
